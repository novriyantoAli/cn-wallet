...
func TestWalletHandler_SetPIN(t *testing.T) {
	t.Run("should set PIN successfully", func(t *testing.T) {
		...
		mockService.On("SetPIN", mock.MatchedBy(func(ctx context.Context) bool {
			return true
		}), userID, mock.MatchedBy(func(r *dto.SetPINRequest) bool {
			return r.CurrentPIN == req.CurrentPIN && r.NewPIN == req.NewPIN
		})).Return(nil)

		reqBody, _ := json.Marshal(req)
		w := httptest.NewRecorder()
		ctx, _ := gin.CreateTestContext(w)
		ctx.Request = httptest.NewRequest("POST", "/wallets/1/set-pin", bytes.NewBuffer(reqBody))
		ctx.Request.Header.Set("Content-Type", "application/json")
		ctx.Params = gin.Params{
			{Key: "user_id", Value: "1"},
		}
...
func TestWalletHandler_VerifyPIN(t *testing.T) {
	t.Run("should verify PIN successfully", func(t *testing.T) {
		...
		mockService.On("VerifyPIN", mock.MatchedBy(func(ctx context.Context) bool {
			return true
		}), userID, req.PIN).Return(nil)

		reqBody, _ := json.Marshal(req)
		w := httptest.NewRecorder()
		ctx, _ := gin.CreateTestContext(w)
		ctx.Request = httptest.NewRequest("POST", "/wallets/1/verify-pin", bytes.NewBuffer(reqBody))
		ctx.Request.Header.Set("Content-Type", "application/json")
		ctx.Params = gin.Params{
			{Key: "user_id", Value: "1"},
		}

...
func TestWalletService_CreateWallet(t *testing.T) {
	...
	t.Run("should return error for invalid PIN length", func(t *testing.T) {
		// Setup
		mockRepo := &testutil.MockWalletRepository{}
		logger := testutil.NewSilentLogger()
		service := NewWalletService(mockRepo, logger)
		...
		assert.Error(t, err)
		assert.Nil(t, wallet)
		assert.Equal(t, "PIN must be exactly 6 digits", err.Error())
	})

	t.Run("should return error for non-numeric PIN", func(t *testing.T) {
		// Setup
		mockRepo := &testutil.MockWalletRepository{}
		logger := testutil.NewSilentLogger()
		service := NewWalletService(mockRepo, logger)
		...
		assert.Error(t, err)
		assert.Nil(t, wallet)
		assert.Equal(t, "PIN must contain only numeric digits", err.Error())
	})
...
func TestWalletService_SetPIN(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("should set PIN successfully", func(t *testing.T) {
		...
		// Mock expectations
		mockRepo.On("GetWalletByUserID", ctx, walletFixture.UserID).Return(walletFixture, nil)
		mockRepo.On("UpdatePIN", ctx, walletFixture.UserID, mock.AnythingOfType("string")).Return(nil)

		// When
		err := service.SetPIN(ctx, walletFixture.UserID, req)
	...
	t.Run("should return error for invalid current PIN", func(t *testing.T) {
		// Setup
		mockRepo := &testutil.MockWalletRepository{}
		logger := testutil.NewSilentLogger()
		service := NewWalletService(mockRepo, logger)
		...
		// Then
		assert.Error(t, err)
		assert.Equal(t, "invalid current PIN", err.Error())
		mockRepo.AssertExpectations(t)
	})

	t.Run("should return error for invalid new PIN format", func(t *testing.T) {
		// Setup
		mockRepo := &testutil.MockWalletRepository{}
		logger := testutil.NewSilentLogger()
		service := NewWalletService(mockRepo, logger)
...
func TestWalletService_VerifyPIN(t *testing.T) {
	ctx := context.Background()

	t.Run("should verify correct PIN successfully", func(t *testing.T) {
		// Setup
		mockRepo := &testutil.MockWalletRepository{}
		logger := testutil.NewSilentLogger()
		service := NewWalletService(mockRepo, logger)
		...
		// When - Note: wallet fixture has PIN "123456" hashed
		err := service.VerifyPIN(ctx, walletFixture.UserID, "123456")
	...
	t.Run("should return error for incorrect PIN", func(t *testing.T) {
		// Setup
		mockRepo := &testutil.MockWalletRepository{}
		logger := testutil.NewSilentLogger()
		service := NewWalletService(mockRepo, logger)
		...
		// When
		err := service.VerifyPIN(ctx, walletFixture.UserID, "000000")
	...
	t.Run("should return error when wallet not found", func(t *testing.T) {
		...
		// When
		err := service.VerifyPIN(ctx, 9999, "123456")

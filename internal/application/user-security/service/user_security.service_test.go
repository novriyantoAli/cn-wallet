...
func setupUserSecurityServiceWithMocks() (UserSecurityService, *testutil.MockUserSecurityRepository) {
	mockRepo := &testutil.MockUserSecurityRepository{}
	logger := testutil.NewSilentLogger()
	service := NewUserSecurityService(mockRepo, logger)
	return service, mockRepo
}

func setupUserSecurityService(mockRepo *testutil.MockUserSecurityRepository) UserSecurityService {
	logger := testutil.NewSilentLogger()
	service := NewUserSecurityService(mockRepo, logger)
	return service
}
...
func TestUserSecurityService_SetPIN(t *testing.T) {
	ctx := context.Background()

	t.Run("should create new security record when none exists", func(t *testing.T) {
		// Setup
		service, mockRepo := setupUserSecurityServiceWithMocks()
		...
		mockRepo.On("Create", ctx, mock.MatchedBy(func(s *entity.UserSecurity) bool {
	...
	t.Run("should update existing PIN", func(t *testing.T) {
		// Setup
		mockRepo := &testutil.MockUserSecurityRepository{}
		service := setupUserSecurityService(mockRepo)
		...
		existingSecurity := &entity.UserSecurity{
			UserID:        1,
	...
	t.Run("should return error when repository fails to create", func(t *testing.T) {
		...
		// Mock expectations
		mockRepo.On("GetByUserID", ctx, req.UserID).Return(nil, nil)
		mockRepo.On("Create", ctx, mock.AnythingOfType("*entity.UserSecurity")).Return(errors.New("create error"))
...
func TestUserSecurityService_VerifyPIN(t *testing.T) {
	ctx := context.Background()
...
func TestUserSecurityService_GetSecurity(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("should return default security info when record not found", func(t *testing.T) {
		...
		assert.NotNil(t, response)
		assert.Equal(t, userID, response.UserID)
		assert.Equal(t, 0, response.FailedAttempt)
		assert.False(t, response.IsLocked)
		mockRepo.AssertExpectations(t)
	})
...
func TestUserSecurityService_IsAccountLocked(t *testing.T) {
	ctx := context.Background()
